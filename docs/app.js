...
function renderPipeline(d){show('pipelineCard');let nodes=[['Token IDs',`[${d.tokens.length}]`],['Token embeddings',shape(d.embedding?.sequence_shape||d.embedding?.shape)],['Transformer Layers',`${d.model.layers} layers · hidden size ${d.model.hidden_size}`],['Final RMSNorm',shape(d.final_hidden_state?.shape)],['LM Head',shape(d.logits?.shape)],['Softmax',`${d.model.vocab_size.toLocaleString()} probabilities`]];$('pipelineCard').innerHTML=`<h2>2 · Transformer pipeline</h2><div class="pipeline">${nodes.map((n,i)=>`<div class="pipeline-box"><div class="pipeline-name">${esc(n[0])}</div><div class="pipeline-shape">${esc(n[1])}</div></div>${i<nodes.length-1?'<div class="arrow">↓</div>':''}`).join('')}</div>`}
